...
namespace FPS_n2 {
	namespace Sceneclass {
					...
					for (int index = 0; index < PlayerMngr->GetPlayerNum(); ++index) {
						...
						if ((PlayerID)index == GetMyPlayerID()) {
							...
							if (m_GameMode != GameMode::Training) {
								...
								m_Replay.SetBack().SetOnce((PlayerID)index).SetMyPlayer(c->GetInputControl(), c->GetMove(), c->GetDamageEvent(), FreeData);
							}
						}
						else {
		...
		//
		void			MainGameScene::LoadResource(void) noexcept {
			auto* SE = SoundPool::Instance();
			//
		...
		void			MainGameScene::SetResource(void) noexcept {
			auto* SE = SoundPool::Instance();
		...
		void			MainGameScene::DisposeResource(void) noexcept {
			auto* SE = SoundPool::Instance();
			//
		...
		//
		void			MainGameScene::LoadChara(const std::string& FolderName, PlayerID ID) noexcept {
			auto* PlayerMngr = Player::PlayerManager::Instance();
			auto* ObjMngr = ObjectManager::Instance();
		...
		void			MainGameScene::LoadWeapon(const std::string& FolderName, PlayerID ID) noexcept {
			auto* ObjMngr = ObjectManager::Instance();
			auto* PlayerMngr = Player::PlayerManager::Instance();
		...
		void MainGameScene::LoadJudge(const std::string& FolderName) noexcept {
			auto* ObjMngr = ObjectManager::Instance();
		...
		void			MainGameScene::Load_Sub(void) noexcept {
			...
			for (int index = 0; index < PlayerMngr->GetPlayerNum(); ++index) {
				LoadChara("Chara", (PlayerID)index);
				LoadWeapon("Bamboo", (PlayerID)index);
			}
			//
			if (m_GameMode != GameMode::Training) {
				for (int index = 0; index < 3; ++index) {
