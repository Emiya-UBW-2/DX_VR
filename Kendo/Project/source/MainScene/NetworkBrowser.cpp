...
namespace FPS_n2 {
	namespace Sceneclass {
		...
		void NetWorkBrowser::Draw(void) noexcept {
			...
			int xp, yp, xs, ys;
			xp = DrawParts->GetScreenY(100);
			yp = DrawParts->GetScreenY(250);
			xs = DrawParts->GetScreenY(500);
			ys = DrawParts->GetScreenY(300);

			int y_h = DrawParts->GetScreenY(30);
			//ラムダ
			auto AddSubBox = [&](int xp1, int yp1, std::function<void()> UpFunc, std::function<void()> DownFunc) {
				int xp2, yp2;
				{
					xp2 = xp1 + DrawParts->GetScreenY(50);
					yp2 = yp1 + y_h;
				...
				{
					yp1 += DrawParts->GetScreenY(50);
					yp2 += DrawParts->GetScreenY(50);
					bool into = IntoMouse(xp1, yp1, xp2, yp2);
					color = (into) ? Black : Gray75;
					DrawBox(xp1, yp1, xp2, yp2, color, TRUE);
					Fonts->Get(FontPool::FontType::Nomal_Edge)->DrawString(y_h, FontHandle::FontXCenter::MIDDLE, FontHandle::FontYCenter::TOP, xp1 + (xp2 - xp1) / 2, yp1, White, Black, "-");
			...
			//
			{
				DrawBox(xp - DrawParts->GetScreenY(10), yp - DrawParts->GetScreenY(10), xp + xs + DrawParts->GetScreenY(10), yp + ys + DrawParts->GetScreenY(10), Gray25, TRUE);//背景
			...
			switch (this->m_Sequence) {
				case SequenceEnum::SelMode:
					if (WindowSystem::SetMsgClickBox(xp, yp + DrawParts->GetScreenY(50), xp + xs, yp + DrawParts->GetScreenY(50) + y_h, Gray75, "クライアントになる")) {
						this->m_IsClient = true;
						this->m_Tick = 1.f;
						this->m_Sequence = SequenceEnum::CheckPreset;
					}
					if (WindowSystem::SetMsgClickBox(xp, yp + DrawParts->GetScreenY(100), xp + xs, yp + DrawParts->GetScreenY(100) + y_h, Gray75, "サーバーになる")) {
				...
				case SequenceEnum::CheckPreset:
					WindowSystem::SetMsgBox(xp, yp + DrawParts->GetScreenY(50), xp + xs, yp + DrawParts->GetScreenY(50) + y_h, Black, "プリセット設定");
					for (int i = 0; i < this->m_NewWorkSetting.GetSize(); i++) {
						auto n = this->m_NewWorkSetting.Get(i);
						if (WindowSystem::SetMsgClickBox(xp, yp + DrawParts->GetScreenY(50)*(i + 2), xp + xs, yp + DrawParts->GetScreenY(50)*(i + 2) + y_h, Gray75, "[%d][%d,%d,%d,%d]", n.UsePort, n.IP.d1, n.IP.d2, n.IP.d3, n.IP.d4)) {
					...
					}
					{
						...
						if (WindowSystem::SetMsgClickBox(xp, yp + DrawParts->GetScreenY(50)*(i + 2), xp + xs, yp + DrawParts->GetScreenY(50)*(i + 2) + y_h, Gray75, "設定を追加する")) {
				...
				case SequenceEnum::Set_Port://ポート
					WindowSystem::SetMsgBox(xp, yp + DrawParts->GetScreenY(50), xp + xs, yp + DrawParts->GetScreenY(50) + y_h, Black, "ポート=[%d-%d]", this->m_NewSetting.UsePort, this->m_NewSetting.UsePort + Player_num - 1);
					AddSubBox(xp, yp + DrawParts->GetScreenY(100), [&]() { this->m_NewSetting.UsePort++; }, [&]() { this->m_NewSetting.UsePort--; });
					if (WindowSystem::SetMsgClickBox(DrawParts->GetScreenY(380), yp + DrawParts->GetScreenY(100), DrawParts->GetScreenY(380) + DrawParts->GetScreenY(120), yp + DrawParts->GetScreenY(100) + y_h, Gray75, "Set")) {
						this->m_Sequence = SequenceEnum::SetIP;//サーバ-は一応いらない
					}
					break;
				case SequenceEnum::SetIP://IP
					WindowSystem::SetMsgBox(xp, yp + DrawParts->GetScreenY(50), xp + xs, yp + DrawParts->GetScreenY(50) + y_h, Black, "IP=[%d,%d,%d,%d]", this->m_NewSetting.IP.d1, this->m_NewSetting.IP.d2, this->m_NewSetting.IP.d3, this->m_NewSetting.IP.d4);
					for (int i = 0; i < 4; i++) {
						...
						AddSubBox(DrawParts->GetScreenY(100 + 70 * i), yp + DrawParts->GetScreenY(100),
								  [&]() {
				...
				case SequenceEnum::SetTick:
					WindowSystem::SetMsgBox(xp, yp + DrawParts->GetScreenY(50), xp + xs, yp + DrawParts->GetScreenY(50) + y_h, Black, "ティック=[%4.1f]", Frame_Rate / this->m_Tick);
					AddSubBox(xp, yp + DrawParts->GetScreenY(100), [&]() { this->m_Tick = std::clamp(this->m_Tick - 1.f, 1.f, 20.f); }, [&]() { this->m_Tick = std::clamp(this->m_Tick + 1.f, 1.f, 20.f); });
					if (WindowSystem::SetMsgClickBox(DrawParts->GetScreenY(380), yp + DrawParts->GetScreenY(100), DrawParts->GetScreenY(380) + DrawParts->GetScreenY(120), yp + DrawParts->GetScreenY(100) + y_h, Gray75, "Set")) {
						this->m_Sequence = SequenceEnum::Matching;
					}
					break;
				case SequenceEnum::Matching:
					WindowSystem::SetMsgBox(xp, yp + DrawParts->GetScreenY(50), xp + xs, yp + DrawParts->GetScreenY(50) + y_h, Black, "他プレイヤー待機中…");
				...
				case SequenceEnum::MainGame:
					WindowSystem::SetMsgBox(xp, yp + DrawParts->GetScreenY(50), xp + xs, yp + DrawParts->GetScreenY(50) + y_h, Black, "通信中!");
					break;
				default:
					break;
			}
