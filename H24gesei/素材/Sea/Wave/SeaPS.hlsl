...
struct PS_OUTPUT
{
    ...
    float4 Color1 : SV_TARGET1; // コースティック
};
...
//水を除いた描画結果
SamplerState g_ScreenColorSampler : register(s2);
Texture2D g_ScreenColorTexture : register(t2);

//水を除いた深度
SamplerState g_ScreenDepthSampler : register(s3);
Texture2D g_ScreenDepthTexture : register(t3);


SamplerState dynamicCubeMapSampler     : register(s4);
TextureCube  dynamicCubeMapTexture     : register(t4);
...
PS_OUTPUT main(PS_INPUT PSInput)
{
	...
	//コースティック(一応)
	{
        float c = dot(normalize(Normal), float3(0.0f, 0.0f, 1.0f)); //輝度を計算する
        c = c * 0.5f + 0.5f; //0.0f ～ 1.0fの範囲内に変換する
        c = pow(c, 100.0f); //陰を適当に強調する
        PSOutput.Color1.rgb = 1.f;
        PSOutput.Color1.a = (1.0f - c) * 1.2f; //白黒反転
    }
    return PSOutput;
}
