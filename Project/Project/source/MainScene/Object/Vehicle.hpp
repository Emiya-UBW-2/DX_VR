	...
	namespace Sceneclass {
		...
		private:
			...
			private:
				...
				b2RevoluteJointDef			m_jointDef;					//ジョイント
			...
			public:
					...
					this->m_Foot.clear();
					{
						...
						for (const auto& w : pUseVeh->Get_b2upsideframe(IsLeft ? 0 : 1)) {
							...
							if (w.second.x() * LorR > 0) {
								...
								this->m_jointDef.Initialize(BodyA, this->m_Foot.back().Get(), anchor);
								this->m_FootWorld->CreateJoint(&this->m_jointDef);
								BodyA = this->m_Foot.back().Get();
							}
						}
						if (!this->m_Foot.empty()) {
							this->m_jointDef.Initialize(BodyA, BodyB, anchor);
							this->m_FootWorld->CreateJoint(&this->m_jointDef);
						}
					}
					{
				...
				void			LateExecute(bool IsLeft, const VhehicleData* pUseVeh, MV1* pTargetObj, const b2Vec2& pGravity, float pWheelRotate, float pSpdAdd) noexcept {
					int LR_t = (IsLeft ? 1 : -1);
					...
					//エフェクト更新
					for (auto& t : this->m_downsideframe) {
						t.EffectExecute(pTargetObj, pSpdAdd);
					}
				}
				//
				void			Dispose(void) noexcept {
