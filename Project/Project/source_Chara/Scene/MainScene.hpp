		...
		public:
			...
			void Set(void) noexcept override {
				...
				for (int i = 0; i < Chara_num; i++) {
					...
					m_ServerCtrl.SetParam(i, c->GetPosBuf());
					m_ClientCtrl.SetParam(i, c->GetPosBuf());
				}
				//登録
				for (int i = 0; i < Vehicle_num; i++) {
			...
			bool Update(void) noexcept override {
				...
				//Input,AI
				{
					...
					//クライアント
					if (this->m_IsClient) {
						m_ClientCtrl.SetMyPlayer(MyInput,
							(!IsRide) ? Chara->GetPosBuf() : Vehicle->GetMove().pos,
							(!IsRide) ? Chara->GetMove().vec : Vehicle->GetMove().vec,
							(!IsRide) ? 0.f : Vehicle->Get_body_yrad()
						);
					...
					//サーバー
						...
						m_ServerCtrl.SetMyPlayer(MyInput,
							(!IsRide) ? Chara->GetPosBuf() : Vehicle->GetMove().pos,
							(!IsRide) ? Chara->GetMove().vec : Vehicle->GetMove().vec,
							(!IsRide) ? 0.f : Vehicle->Get_body_yrad()
						);
						...
						for (int i = 0; i < Chara_num; i++) {
							...
							if (i == GetMyPlayerID()) {
								...
								if (!IsRide) {
									...
									auto diff = (tmp.PosBuf - c->GetPosBuf());
									auto siz = diff.size();
									auto vec = c->GetMove().vec;
									if (((1.0f*Scale_Rate) < siz) && (siz < (2.0f*Scale_Rate))) {
										c->SetPosBufOverRide(tmp.PosBuf, vec);
									}
									//y方向だとベクトルリセット
			...
			void UI_Draw(void) noexcept  override {
				...
				for (int i = 0; i < Vehicle_num; i++) {
					...
					if (0.f < campos.z() && campos.z() < 1.f) {
						...
						Fonts->Get(siz, FontPool::FontType::HUD_Edge).Get_handle().DrawStringFormat(xp + xs, yp + ys, color, White, "%dm", (int)((v->GetMatrix().pos() - ((!IsRide) ? Chara->GetMatrix().pos() : Vehicle->GetMatrix().pos())).size() / Scale_Rate));
						//リセット
						campos.z(-1.f);
						v->SetCameraPosition(campos);
					}
