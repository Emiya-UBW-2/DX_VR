	...
	namespace Sceneclass {
		...
		void				GunClass::UpdateGunAnims(void) noexcept {
			...
			case GunAnimeID::Reload:
				if (this->m_isMagSuccess ? (GetGunAnimePer(GetGunAnime()) >= 0.75f) : (GetGunAnimePer(GetGunAnime()) >= 1.f)) {
					...
					case RELOADTYPE::MAG:
						this->m_Capacity = std::clamp(this->m_ReloadAddAmmo, 0, this->GetAmmoAll());//É}ÉKÉWÉìëïìU
						SetGunAnime(GunAnimeID::ReloadEnd);
						break;
					case RELOADTYPE::AMMO:
						this->m_Capacity = std::clamp(this->m_Capacity + 1, 0, this->GetAmmoAll());//É}ÉKÉWÉìëïìU
		...
		void				GunClass::Init_Sub(void) noexcept {
			...
			if (this->m_MagazinePtr) {
				...
				this->m_CartFall.Init(GetMagazinePtr()->GetModSlot().GetModData()->GetAmmoSpecMagTop()->GetPath(), 4);	//ëïìUÇµÇΩÉ}ÉKÉWÉìÇÃíeÇ…çáÇÌÇπÇƒñÚ‰∞ê∂ê¨
			}
			SetGunAnime(GunAnimeID::None);
			UpdateGunAnime(false, true);
			this->m_Capacity = std::clamp(GetAmmoAll(), 0, this->GetAmmoAll());//É}ÉKÉWÉìëïìU
			if (this->m_MagazinePtr) {
				this->m_ChamberAmmoData = GetMagazinePtr()->GetModSlot().GetModData()->GetAmmoSpecMagTop();//É}ÉKÉWÉìÇÃàÍî‘è„ÇÃíeÉfÅ[É^ÇÉ`ÉÉÉìÉoÅ[ÉCÉì
			}
		}
