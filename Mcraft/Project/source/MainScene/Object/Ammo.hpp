	...
	namespace Sceneclass {
		...
		public:
			void			Put(std::shared_ptr<AmmoDataClass> pAmmoData, const Vector3DX& pos_t, const Vector3DX& pVec, int pMyID) {
				...
				this->pos = pos_t;
				this->repos = pos_t;
				SetMove().SetAll(pos_t, pos_t, pos_t, pVec, Matrix3x3DX::identity(), Matrix3x3DX::identity());
			...
			void			Ricochet(const Vector3DX& position, const Vector3DX& normal) {
				...
				this->m_IsDrawHitUI = false;
				this->m_RicochetCnt++;
				SetMove().SetVec((this->GetMove().GetVec() + normal * ((Vector3DX::Dot(this->GetMove().GetVec(), normal)) * -2.0f)).normalized());
