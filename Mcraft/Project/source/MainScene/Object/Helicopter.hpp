...
namespace FPS_n2 {
	namespace Objects {
		...
		class TeamHelicopterObj : public BaseObject {
			static const PlayerID								m_MyPlayerID{ -3 };//車両はとりあえず-1
		...
		private:
			...
			const char* GetFrameStr(int id) noexcept override { return HeliFrameName[id]; }
		public:
			TeamHelicopterObj(void) noexcept { this->m_objType = static_cast<int>(ObjType::Helicopter); }
			virtual ~TeamHelicopterObj(void) noexcept {}
		public:
			void			SetMat(const Vector3DX& pos, const Matrix3x3DX& mat) noexcept {
				SetMove().SetAll(pos, pos, pos, Vector3DX::zero(), mat, mat);
				SetMove().Update(0.0f, 0.0f);
		...
		public:
			void				Init_Sub(void) noexcept override {
				...
				SetActive(true);
				auto* SE = SoundPool::Instance();
				SE->Get(SoundType::SE, static_cast<int>(SoundEnum::Heli2))->Play3D(GetMove().GetPos(), 50.0f * Scale3DRate, DX_PLAYTYPE_LOOP);
				m_PosR = Vector3DX::vget(0.0f, -20.0f, 0.0f) * Scale3DRate;
				m_Yrad = 0.0f;
				m_YradR = 0.0f;
				m_TargetPos = Vector3DX::vget(GetRandf(16.f), 0.0f, GetRandf(16.f)) * 2.f * Scale3DRate;
			...
			void				Dispose_Sub(void) noexcept override {
				auto* SE = SoundPool::Instance();
				SE->Get(SoundType::SE, static_cast<int>(SoundEnum::Heli2))->StopAll();
			}
		};
	}
}
