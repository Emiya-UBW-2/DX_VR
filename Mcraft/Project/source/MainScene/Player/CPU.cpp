	...
	namespace AIs {
		...
		void		AIControl::Repop(void) noexcept {
			...
			if (this->m_IsLeftHeli) {
				ZVec = PlayerMngr->GetHelicopter(0)->GetMove().GetMat().xvec() * -1.0f;
				Pos = PlayerMngr->GetHelicopter(0)->GetObj().GetFrameLocalWorldMatrix(PlayerMngr->GetHelicopter(0)->GetFrame(static_cast<int>(Objects::HeliFrame::Rappelling2))).pos();
			}
			else {
				ZVec = PlayerMngr->GetHelicopter(0)->GetMove().GetMat().xvec();
				Pos = PlayerMngr->GetHelicopter(0)->GetObj().GetFrameLocalWorldMatrix(PlayerMngr->GetHelicopter(0)->GetFrame(static_cast<int>(Objects::HeliFrame::Rappelling1))).pos();
			}
		...
		const InputControl AIControl::Update(void) noexcept {
			...
			if (MyChara->IsAlive()) {
				...
				if (MyChara->GetIsRappelling()) {
					...
					MoveInfoData.pos = Pos - PlayerMngr->GetHelicopter(0)->GetMove().GetMat().yvec()*(60.0f * this->m_RapeTimer);
				...
				if (this->m_RepopTimer > 5.0f) {
					...
					if (PlayerMngr->GetHelicopter(0)->GetIsActiveRappelling()) {
						this->m_IsLeftHeli = (PlayerMngr->GetHelicopter(0)->PopSpawnPoint() % 2) == 0;
						Repop();
					}
				}
			}
