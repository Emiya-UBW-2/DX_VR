	...
	namespace Sceneclass {
...
#if NOTUSEPATHCHECKER
					auto* BackGround = BackGround::BackGroundClass::Instance();
					// 隣接するポリゴンが既に経路探索処理が行われていて、且つより距離の長い経路となっている場合は何もしない
					auto& Unit = BackGround->GetBuildData().at(PUnit->PolyIndex);

					auto trisdistance = PUnit->TotalDistance +
						(BackGround->GetBuildData().at(Unit.GetLinkPolyIndex(tris)).GetMatrix().pos() + Unit.GetMatrix().pos()).magnitude();
		...
		public:
			Vector3DX GetNextPoint(const Vector3DX& NowPosition, int* TargetPathPlanningIndex) const {
				...
				if (NowIndex != this->GoalUnit->GetPolyIndex()) {																	// 現在乗っているポリゴンがゴール地点にあるポリゴンの場合は処理を分岐
					...
					return BackGround->GetBuildData().at(*TargetPathPlanningIndex).GetMatrix().pos();
				}
				else {
					// 方向は目標座標
		...
		public:
			bool Init(Vector3DX StartPos, Vector3DX GoalPos) {
				...
				this->UnitArray.resize(BackGround->GetBuildData().size());			// 経路探索用のポリゴン情報を格納するメモリ領域を確保、初期化
				...
				while (true) {
					...
					while (true) {
						// ポリゴンの辺の数だけ繰り返し
						for (int K = 0; K < 4; K++) {
							int Index = BackGround->GetBuildData().at(PUnit->GetPolyIndex()).GetLinkPolyIndex(K);
		...
		private:
			...
			void		ChangePoint() noexcept {
				...
				auto GetRandomMyPos = [&]() {
					auto* BackGround = BackGround::BackGroundClass::Instance();
					if (SelList.size() == 0) {
						for (auto& C : BackGround->GetBuildData()) {
							if (C.GetMeshSel() < 0) { continue; }
							auto Vec = C.GetMatrix().pos() - MyPos; Vec.y = (0.f);
							if (Vec.magnitude() < 10.f * Scale_Rate) {
								SelList.emplace_back(static_cast<int>(&C - &BackGround->GetBuildData().front()));
							}
						}
					}
					MyPos = BackGround->GetBuildData().at(SelList.at(GetRand((int)SelList.size() - 1))).GetMatrix().pos();
					};
				...
				if (GetLengthToTarget() > 20.f * Scale_Rate) {
					...
					Target = BackGround->GetBuildData().at(SelList.at(GetRand((int)SelList.size() - 1))).GetMatrix().pos();
				}
			...
			void		Repop(bool CanRepop) noexcept {
				...
				Vector3DX pos_t;
				while (true) {
					pos_t = BackGround->GetBuildData().at(static_cast<size_t>(GetRand(static_cast<int>(BackGround->GetBuildData().size()) - 1))).GetMatrix().pos();
					...
					if (BackGround->CheckLinetoMap(TgtPos, &EndPos)) {
						break;
					}
				}
				...
				if (BackGround->CheckLinetoMap(pos_t + Vector3DX::up() * -10.f*Scale_Rate, &EndPos)) {
					pos_t = EndPos;
				}
				//*/
