...
namespace FPS_n2 {
	namespace Sceneclass {
		void			ItemObjClass::SetFall(const Vector3DX& pos, const Vector3DX& vec) noexcept {
			...
			this->m_move.SetAll(pos, pos, pos, vec, Matrix3x3DX::identity(), Matrix3x3DX::identity());
		}
		void			ItemObjClass::FirstExecute(void) noexcept {
			...
			if (this->m_IsActive) {
				if (this->m_yAdd != 0.f) {
					...
					Vector3DX vec = this->m_move.GetVec();
					...
					if (this->m_BackGround->CheckLinetoMap(this->GetMove().GetRePos(), &EndPos, false, &Normal)) {
						...
						this->m_move.SetVec(this->m_move.GetVec() + Normal * (Vector3DX::Dot(Normal, this->GetMove().GetVec() * -1.f)) * 2.f);
						if (Normal.y > 0.5f) {
							this->m_yAdd *= -0.5f;
						}
					}
					...
					if (this->m_move.GetPos().y == 0.f) {
						this->m_yAdd = 0.f;
					}
				}

				this->m_move.SetMat(Matrix3x3DX::RotAxis(Vector3DX::up(), deg2rad(1.f * 60.f / DrawParts->GetFps())) * this->m_move.GetMat());
