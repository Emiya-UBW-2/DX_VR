...
namespace FPS_n2 {
	namespace Sceneclass {
		...
		// 空間上から画面上への変換
		extern const Vector3DX Convert2DtoDisp(const Vector3DX& Pos2D) noexcept;
		// ベクトルのxy間の角度を取得
		extern const float GetRadVec2Vec(const Vector3DX& vec1, const Vector3DX& vec2) noexcept;
		// 空間上のタイルごとのサイズを取得
		extern const float Get2DSize(float value) noexcept;
		// 画面上のタイルごとのサイズを取得
		extern const int GetDispSize(float value) noexcept;
	};
};
