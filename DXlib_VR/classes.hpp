...
public:
	...
	public:
		...
		frames fps_view;
		//事前読み込み
		static void set_vehicles_pre(const char* name, std::vector<Vehcs>* vehcs, const bool& Async) {
			...
			if (hFind != INVALID_HANDLE_VALUE) {
				do {
					if ((win32fdt.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && (win32fdt.cFileName[0] != '.')) {
						vehcs->resize(vehcs->size() + 1);
						vehcs->back().name = win32fdt.cFileName;
					}
		...
		static void set_vehicles(std::vector<Vehcs>* vehcs) {
			...
			for (auto& t : (*vehcs)) {
				...
				for (int i = 0; i < t.obj.frame_num(); i++) {
					...
					if (p.find("視点", 0) != std::string::npos) {
						t.fps_view.first = i;
