...
PS_OUTPUT main(PS_INPUT PSInput)
{
    ...
    float TextureDepth = g_DepthMapTexture.Sample(g_DepthMapSampler, DepthTexCoord).r;

	// テクスチャに記録されている深度よりＺ値が大きかったら奥にある
	if (PSInput.LPPosition.z > (TextureDepth + 0.1f) && TextureDepth > 0.f) {
		/*
		PSOutput.Color0.r = clamp(
			(PSInput.LPPosition.z - (TextureDepth + 0.1f)) / 10.f,
			0.f, 1.f);
		//*/
	...
	// 深度バッファテクスチャから深度を取得( +補正値 )
	float TextureDepth2 = g_DepthMapTexture2.Sample(g_DepthMapSampler2, DepthTexCoord).r;

	// テクスチャに記録されている深度よりＺ値が大きかったら奥にある
    if (PSInput.LPPosition2.z > (TextureDepth2 + 0.1f) && TextureDepth2 > 0.f)
    {
		/*
        PSOutput.Color0.r = clamp(
            (PSInput.LPPosition2.z - (TextureDepth2 + 0.1f)) / 10.f,
            0.f, 1.f);
		//*/
        PSOutput.Color0.r = 1.f;
    }
