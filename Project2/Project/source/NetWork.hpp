...
namespace FPS_n2 {
	namespace Sceneclass {
		...
		public:
			...
			Vector3DX		PosBuf;				//4 * 3	= 12byte
			Vector3DX		VecBuf;				//4 * 3	= 12byte
			Vector3DX		radBuf;				//4 * 3	= 12byte
		...
		public:
			...
			void			SetMyPlayer(const InputControl& pInput, const Vector3DX& pos_t, const Vector3DX& pVec, const Vector3DX& prad, double pFrame, const DamageEvent* pDamage, unsigned long long pDamageSwitch) noexcept {
			...
			const auto		GetNowServerPlayerData(int pPlayerID, bool isyradReset) noexcept {
				...
				if (isyradReset) {
					auto radvec = Lerp(Matrix4x4DX::RotAxis(Vector3DX::up(), this->m_PrevServerData.PlayerData[pPlayerID].radBuf.y).zvec(), Matrix4x4DX::RotAxis(Vector3DX::up(), this->m_ServerDataCommon.PlayerData[pPlayerID].radBuf.y).zvec(), Per).normalized();
			...
			virtual void	SetParam(int pPlayerID, const Vector3DX& pos_t) noexcept {
		...
		public:
			...
			void			SetParam(int pPlayerID, const Vector3DX& pos_t) noexcept override {
			...
			bool			Execute(void) noexcept override {
				...
				case 0:
					...
					if (this->m_NetWork.first.Recv(&tmpData)) {
						...
						this->m_CannotConnectTimer += 1.f / DrawParts->GetFps();
