		...
		void			CheckTreetoSquare(
			...
			for (auto& l : tree) {
				if (!l.fall_flag) {
					...
					size_t cnt = 0;
					cnt += ((Vector3DX::Cross((p0 - p1), l.pos - p1)).y >= 0);
					cnt += ((Vector3DX::Cross((p1 - p2), l.pos - p2)).y >= 0);
					cnt += ((Vector3DX::Cross((p2 - p3), l.pos - p3)).y >= 0);
					cnt += ((Vector3DX::Cross((p3 - p0), l.pos - p0)).y >= 0);
					if (cnt == 4) {
	...
	public:
		...
		void	Init(const MV1* MapCol, const std::vector<Builds>& BGBuild) noexcept {
			...
			for (auto& t : tree) {
				...
				t.mat = Matrix4x4DX::GetScale(Vector3DX::vget(scale, scale, scale));
				while (true) {
					t.pos = Vector3DX::vget(
					...
					for (const auto& bu : BGBuild) {
						if (bu.GetMeshSel() >= 0) {
							...
							if (pos_p.magnitude() < 20.f*Scale_Rate) {
								isnear = true;
								break;
							}
						}
				...
				auto res = MapCol->CollCheck_Line(t.pos + Vector3DX::vget(0.f, 10.f*Scale_Rate, 0.f), t.pos + Vector3DX::vget(0.f, -10.f*Scale_Rate, 0.f));
				if (res.HitFlag == TRUE) {
		...
		void	Draw(bool isSetFog) noexcept {
			...
			for (auto& l : tree) {
				...
				if (CheckCameraViewClip_Box(
					(l.pos + Vector3DX::vget(-20, 0, -20)*Scale_Rate).get(),
					...
					if (LengthtoCam.magnitude() > Farlimit && isSetFog) {
						...
						if (isSetFog) {
							l.obj.material_AlphaTestAll(true, DX_CMP_GREATER, (int)(128.f + 127.f*(Farlimit2 - LengthtoCam.magnitude()) / Farlimit2));
						}
						else {
