...
namespace FPS_n2 {
	namespace Sceneclass {
		...
		public:			//getter
			...
			//銃反動
			void		FireReaction(Matrix4x4DX* mat_t) const noexcept {
				(*mat_t) *= Matrix4x4DX::RotAxis(mat_t->xvec(), this->m_ShotRadAdd.x) * Matrix4x4DX::RotAxis(mat_t->zvec(), this->m_ShotRadAdd.z);
			}
		public:
			//角度指示
			void		ExecuteGunFrame(float pGunXrad, float pGunYrad, MV1* obj_body_t, MV1* col_body_t) noexcept {
				...
				if (id.GetFrameID() >= 0) {
					obj_body_t->SetFrameLocalMatrix(id.GetFrameID(), Matrix4x4DX::Mtrans(Vector3DX::forward() * (this->m_Recoil * 0.5f * Scale_Rate)) * Matrix4x4DX::Mtrans(id.GetFrameLocalPosition().pos()));
					col_body_t->SetFrameLocalMatrix(id.GetFrameID(), Matrix4x4DX::Mtrans(Vector3DX::forward() * (this->m_Recoil * 0.5f * Scale_Rate)) * Matrix4x4DX::Mtrans(id.GetFrameLocalPosition().pos()));
				}
