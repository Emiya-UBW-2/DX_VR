				...
				int size = y_r(64);
				{
					...
					for (auto& I : Player.GetInventorys()) {
						...
						for (auto& xo : I) {
							...
							for (auto& yo : xo) {
								int y = (int)(&yo - &xo.front());
								DrawBox(xp + x * size, yp + y * size, xp + (x + 1) * size, yp + (y + 1) * size, Gray50, FALSE);
							}
						}
						for (auto& xo : I) {
							int x = (int)(&xo - &I.front());
											...
											if (Ptr) {
												if (!m_Drag) {
													...
													if (this->m_Deletekey.trigger()) {
														//アイテム放出
														auto item = std::make_shared<ItemClass>();
														ObjMngr->AddObject(item);
														ObjMngr->LoadModel(item, item, "data/model/item/");

														//item->SetMapCol(this->m_BackGround);
														item->Init();
						...
						std::string InfoStr;
						{
							...
							while (true) {
								...
								if (item) {
									...
									if (ip->IsActive()) {
										auto Len = (ip->GetMove().pos - MyPos).magnitude();
